...
export default function ReservationsTab() {
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: "pendente" | "confirmado" | "cancelado" | "concluido" }) => {
                  ...
                  <tr key={r.id} className="border-b border-border transition-colors hover:bg-secondary/30">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        {r.status === "pendente" && (
                          <button
                            onClick={() => updateStatus.mutate({ id: r.id, status: "confirmado" })}
                        ...
                        {r.status !== "cancelado" && r.status !== "concluido" && (
                          <button
                            onClick={() => updateStatus.mutate({ id: r.id, status: "cancelado" })}
                            disabled={updateStatus.isPending}
                            title="Cancelar"
